...
use Guzzle\Http\Promise\FulfilledPromise;
...
class StreamHandler
{
    ...
    public function __invoke(RequestInterface $request, array $options): PromiseInterface
    {
        ...
        $startTime = isset($options['on_stats']) ? \Guzzle\Http\Functions::_current_time() : null;
        ...
        try {
            ...
            return \Guzzle\Http\Promise\rejection_for($e);
        }
    }
    ...
    private function invokeStats(
        ...
        if (isset($options['on_stats'])) {
            $stats = new TransferStats(
                $request,
                $response,
                \Guzzle\Http\Functions::_current_time() - $startTime,
                $error,
                []
            );
    ...
    private function createResponse(
        ...
        $headers = \Guzzle\Http\Functions::headers_from_lines($hdrs);
        ...
        if (isset($options['on_headers'])) {
            try {
                ...
                return \Guzzle\Http\Promise\rejection_for($ex);
            }
        }
    ...
    private function add_proxy(RequestInterface $request, array &$options, $value, array &$params): void
    {
        if (!\is_array($value)) {
            ...
            if (isset($value[$scheme])) {
                if (!isset($value['no'])
                    || !\Guzzle\Http\Functions::is_host_in_noproxy(
    ...
    private function add_verify(RequestInterface $request, array &$options, $value, array &$params): void
    {
        if ($value === true) {
            ...
            if (PHP_VERSION_ID < 50600) {
                $options['ssl']['cafile'] = \Guzzle\Http\Functions::default_ca_bundle();
            }
    ...
    private function add_debug(RequestInterface $request, array &$options, $value, array &$params): void
    {
        ...
        $value = \Guzzle\Http\Functions::debug_resource($value);
